...
# Declare Variables
log_folder="${EXUI_LOG_FOLDER:=/var/log}"
...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/FakeErrorX/EXUI/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/fakeerrorx/EXUI/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    systemctl disable ex-ui
    rm /etc/systemd/system/ex-ui.service -f
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/fakeerrorx/EXUI/master/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    if [[ $twoFactorConfirm != "y" && $twoFactorConfirm != "Y" ]]; then
        /usr/local/ex-ui/ex-ui setting -username ${config_account} -password ${config_password} -resetTwoFactor false >/dev/null 2>&1
    else
        /usr/local/ex-ui/ex-ui setting -username ${config_account} -password ${config_password} -resetTwoFactor true >/dev/null 2>&1
        echo -e "Two factor authentication has been disabled."
    fi
    
    echo -e "Panel login username has been reset to: ${green} ${config_account} ${plain}"
    echo -e "Panel login password has been reset to: ${green} ${config_password} ${plain}"
    echo -e "${green} Please use the new login username and password to access the ex-ui panel. Also remember them! ${plain}"
    confirm_restart
}
...
reset_webbasepath() {
    ...
    /usr/local/ex-ui/ex-ui setting -webBasePath "${config_webBasePath}" >/dev/null 2>&1
...
reset_config() {
    ...
    /usr/local/ex-ui/ex-ui setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(/usr/local/ex-ui/ex-ui setting -show true)
    if [[ $? != 0 ]]; then
    ...
    local existing_cert=$(/usr/local/ex-ui/ex-ui setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        /usr/local/ex-ui/ex-ui setting -port ${port}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            LOGI "ex-ui Started Successfully"
        else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "ex-ui and xray stopped successfully"
        else
...
restart() {
    systemctl restart ex-ui
    sleep 2
    check_status
    if [[ $? == 0 ]]; then
        LOGI "ex-ui and xray Restarted successfully"
    else
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "ex-ui Set to boot automatically on startup successfully"
    else
        LOGE "ex-ui Failed to set Autostart"
    fi
...
disable() {
    systemctl disable ex-ui
    if [[ $? == 0 ]]; then
        LOGI "ex-ui Autostart Cancelled successfully"
    else
        LOGE "ex-ui Failed to cancel autostart"
    fi
...
show_log() {
    ...
    0)
        ...
        ;;
    1)
        journalctl -u ex-ui -e --no-pager -f -p debug
        if [[ $# == 0 ]]; then
            before_show_menu
        fi
        ;;
...
update_shell() {
    wget -O /usr/bin/ex-ui -N https://github.com/FakeErrorX/EXUI/raw/main/ex-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_status() {
    if [[ ! -f /etc/systemd/system/ex-ui.service ]]; then
        return 2
    fi
    temp=$(systemctl status ex-ui | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    temp=$(systemctl is-enabled ex-ui)
        ...
        if [ -z "$domains" ]; then
            ...
            if echo "$domains" | grep -qw "$domain"; then
                ...
                if [[ -f "${webCertFile}" && -f "${webKeyFile}" ]]; then
                    /usr/local/ex-ui/ex-ui cert -webCert "$webCertFile" -webCertKey "$webKeyFile"
...
ssl_cert_issue() {
    ...
    *)
    ...
    reloadCmd="ex-ui restart"

    LOGI "Default --reloadcmd for ACME is: ${yellow}ex-ui restart"
    LOGI "This command will run on every certificate issue and renew."
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; ex-ui restart"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
        1)
            LOGI "Reloadcmd is: systemctl reload nginx ; ex-ui restart"
            reloadCmd="systemctl reload nginx ; ex-ui restart"
            ;;
        2)  
            LOGD "It's recommended to put ex-ui restart at the end, so it won't raise an error if other services fails"
            read -rp "Please enter your reloadcmd (example: systemctl reload nginx ; ex-ui restart): " reloadCmd
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(/usr/local/ex-ui/ex-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(/usr/local/ex-ui/ex-ui setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(/usr/local/ex-ui/ex-ui setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(/usr/local/ex-ui/ex-ui setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(/usr/local/ex-ui/ex-ui setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    1)
        if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
            ...
            /usr/local/ex-ui/ex-ui setting -listenIP "${config_listenIP}" >/dev/null 2>&1
        ...
        ;;
    2)
        /usr/local/ex-ui/ex-ui setting -listenIP 0.0.0.0 >/dev/null 2>&1
...
show_menu() {
    echo -e "
╔────────────────────────────────────────────────╗
│   ${green}EXUI Panel Management Script${plain}                │
